...
/// The memory graph stores URI-keyed nodes with typed edges, automatic snapshot versioning,
...
public interface IMemoryGraph
{
    ...
    /// the old content is snapshotted before the node is updated. Snapshots are pruned to last 10 per URI.
    /// </summary>
    ...
    /// <summary>
    /// Deletes a node and all associated data: its edges (as source or target) and its snapshots.
    /// Invalidates the glossary cache for the affected Anima.
    /// </summary>
    ...
    /// <summary>
    /// Returns the snapshot history for a node in descending order (newest first).
    /// </summary>
