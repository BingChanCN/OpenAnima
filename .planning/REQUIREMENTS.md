...
- [ ] **WORK-01**: Every tool step executes against an explicit workspace root rather than ambient process state
...
- [ ] **WORK-05**: Every tool result records workspace root and enough metadata for replay and audit

### Run Inspection & Observability

- [ ] **OBS-01**: User can inspect a per-run timeline showing step start, completion, cancellation, and failure events
- [ ] **OBS-02**: User can inspect per-step inputs, outputs, errors, durations, and linked artifacts
- [ ] **OBS-03**: User can inspect why a node ran, including upstream trigger and downstream fan-out visibility
- [ ] **OBS-04**: Developer can correlate logs, traces, and tool events by run ID and step ID during debugging

### Artifact & Memory Foundation

- [ ] **ART-01**: System can persist intermediate notes, reports, and final outputs as durable artifacts linked to run and step records
- [ ] **ART-02**: User can inspect run artifacts from the run inspector with source linkage back to the generating step
- [ ] **MEM-01**: System can store retrieval records derived from artifacts with provenance metadata including source artifact, step, and timestamp
