...
must_haves:
  truths:
    - "PortType enum defines Text and Trigger with associated color hex values (#4A90D9 blue, #E8943A orange) in XML doc comments"
...
<task type="auto">
  ...
  <action>
    ...
    1. **PortType.cs** — Enum with two values:
       - `Text = 0` with XML doc comment including visual color `#4A90D9` (blue)
       - `Trigger = 1` with XML doc comment including visual color `#E8943A` (orange)
