...
|   +-- {MM}-{DD}-{name}/
|       +-- task.json
|-- planning/           # Canonical active milestone and roadmap docs
|   +-- README.md           # Planning entry point
|   +-- current-milestone.md # Current unfinished milestone
...
If the task depends on the active milestone or unfinished phases, also read:

```bash
cat .trellis/planning/README.md
cat .trellis/planning/current-milestone.md
```
...
### 3. planning/ - Active Planning

**Purpose**: Canonical home for current milestone, active roadmap context, and links to ongoing phase tasks.

**Structure**:
```
planning/
|-- README.md              # Entry point for active planning
|-- current-milestone.md   # Current unfinished milestone summary
```

**When to update**:
- [OK] Current milestone changes
- [OK] Active phase ownership moves into Trellis tasks
- [OK] Legacy `.planning/` status needs a new canonical Trellis reference
...
2. **During development**:
   ...
   - Split work across multiple agents only when subtasks are independent and ownership does not overlap; otherwise keep execution serialized
...
| Parallelizable task planning | `guides/parallel-agent-thinking-guide.md` |
