...
public class SsrfGuardTests
{
    ...
    {
        var blocked = SsrfGuard.IsBlocked("http://localhost/path", out var reason);
    ...
    {
        var blocked = SsrfGuard.IsBlocked("http://127.0.0.1/api", out var reason);
    ...
    {
        var blocked = SsrfGuard.IsBlocked("http://127.0.0.255/", out _);
        Assert.True(blocked);
    }
    ...
    {
        var blocked = SsrfGuard.IsBlocked("http://[::1]/path", out _);
        Assert.True(blocked);
    }
    ...
    {
        var blocked = SsrfGuard.IsBlocked(url, out _);
    ...
    public void IsBlocked_172_15_0_1_ReturnsFalse_JustOutsideRange()
    {
        ...
        var blocked = SsrfGuard.IsBlocked("http://172.15.0.1/", out _);
    ...
    {
        var blocked = SsrfGuard.IsBlocked("http://192.168.1.1/", out _);
        Assert.True(blocked);
    }
    ...
    {
        var blocked = SsrfGuard.IsBlocked("http://169.254.1.1/", out _);
        Assert.True(blocked);
    }
    ...
    {
        var blocked = SsrfGuard.IsBlocked("http://1.1.1.1/", out _);
    ...
    {
        var blocked = SsrfGuard.IsBlocked("http://8.8.8.8/", out _);
    ...
    {
        var blocked = SsrfGuard.IsBlocked("not-a-valid-url", out var reason);
    ...
    {
        var blocked = SsrfGuard.IsBlocked("", out var reason);
